test_that("assemble result tables", {
  ...
  expect_equal(ncol(tables$result_table_conc_levels), 13)
  ...
  expect_equal(ncol(tables$result_table_obs), 7)
  expect_equal(colnames(tables$result_table_obs),
               c("substance", "concentration", "measurement", "percentage_bias", "response_factor",
                 "RF_within_thres", "final_linear_range"))
})
